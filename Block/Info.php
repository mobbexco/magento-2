    ...
    protected function _prepareSpecificInformation($transport = null)
    {
        ...
        if($cards){
            ...
            foreach ($cards as $key => $card) {
                if($card['source_name'] !== 'multicard'){
                    ...
                    $data[(string) __('Card '.($key + 1).' Installment')] = isset($card['source_installment']) ? json_decode($card['source_installment'], true)['description'] . ' ' . json_decode($card['source_installment'], true)['count'] . ' cuota/s' : '';
            ...
            $data[(string) __('Source Info')] = isset($mobbexData['source_number']) ? $mobbexData['source_number'] : '';
            $data[(string) __('Source Installment')] = isset($mobbexData['source_installment']) ? json_decode($mobbexData['source_installment'], true)['description'] . ' ' . json_decode($mobbexData['source_installment'], true)['count'] . ' cuota/s' : '';
        }
        ...
        if ($this->_state->getAreaCode() == 'adminhtml') {
            ...
            $data = array_merge($data, [
                (string) __('Risk Analisys') => isset($mobbexData['risk_analysis']) ? $mobbexData['risk_analysis'] : '',
